...
const fmtNum = (n) => {
    ...
    const converted = convertFromVND(Number(n), cur, rates);
    const frac = (cur === 'VND' || cur === 'JPY') ? 0 : 2;
    return converted.toLocaleString('vi-VN', { maximumFractionDigits: frac });
};
const getCurLabel = () => getDisplayCurrency();
...
const PDF_LABELS = {
    vi: {
        ...
        catCol: 'Hạng mục', amountCol: 'Thành tiền', laborDiary: 'Nhân công (nhật ký)', matDiary: 'Vật tư (nhật ký)', grandTotal: 'TỔNG CỘNG',
    ...
    en: {
        ...
        catCol: 'Category', amountCol: 'Amount', laborDiary: 'Labor (diary)', matDiary: 'Material (diary)', grandTotal: 'GRAND TOTAL',
    ...
    ede: {
        ...
        catCol: 'Mtă', amountCol: 'Prăk', laborDiary: 'Mnuih (hdro)', matDiary: 'Mnă (hdro)', grandTotal: 'ABŎH PRĂK',
        ...
        try {
            ...
            if (includeDiary) {
                ...
                if (diaryData.length === 0) {
                    ...
                    doc.setFontSize(10);
                    doc.setFont(fontName, 'bold');
                    doc.text(`${P.totalLabor}: ${fmtNum(diaryLaborTotal)} | ${P.totalMat}: ${fmtNum(diaryMatTotal)} | ${P.totalAll}: ${fmtNum(diaryLaborTotal + diaryMatTotal)} ${getCurLabel()}`, margin + 5, y);
                    y += 8;
                }
            }
            ...
            if (includeConsum) {
                ...
                if (consumData.length === 0) {
                    ...
                    summaryRows.push([
                        ...
                        { content: fmtNum(grandTotal) + ' ' + getCurLabel(), styles: { fontStyle: 'bold' } }
                    ]);
