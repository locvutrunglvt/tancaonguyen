            ...
            {showUserModal && (
                <div className="modal-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
                    <div className="modal-content" style={{ background: 'white', padding: '40px', borderRadius: '30px', width: '100%', maxWidth: '500px' }}>
                        <h3 style={{ marginBottom: '25px', color: 'var(--coffee-dark)' }}>{isEditing ? t.update + ' ' + t.users?.toLowerCase() : t.user_add_btn}</h3>
                        <form onSubmit={handleSaveUser} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                            ...
                            <div className="permissions-section" style={{ background: '#f8fafc', padding: '15px', borderRadius: '15px', border: '1px solid #e2e8f0' }}>
                                <label style={{ fontSize: '13px', fontWeight: 600, color: 'var(--coffee-dark)', marginBottom: '10px', display: 'block' }}>
                                    <i className="fas fa-shield-alt"></i> {t.user_perm_title}
                                </label>
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px' }}>
                                        <input type="checkbox" checked disabled /> {t.user_perm_view}
                                    </label>
                                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px' }}>
                                        <input type="checkbox" checked={userForm.role === 'Admin'} disabled /> {t.user_perm_add}
                                    </label>
                                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px' }}>
                                        <input type="checkbox" checked={userForm.role === 'Admin' || userForm.role === 'Farmer'} disabled /> {t.user_perm_edit}
                                    </label>
                                    <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px' }}>
                                        <input type="checkbox" checked={userForm.role === 'Admin'} disabled /> {t.user_perm_delete}
                                    </label>
                                </div>
                            </div>

                            <div className="modal-actions" style={{ display: 'flex', gap: '10px', marginTop: '10px' }}>
                                <button type="submit" className="btn-primary" style={{ flex: 1 }}>{isEditing ? t.update : t.add}</button>
                                <button type="button" onClick={() => setShowUserModal(false)} className="btn-primary" style={{ flex: 1, background: '#f1f5f9', color: '#475569' }}>{t.cancel}</button>
                            </div>
                        </form>
                    </div>
                </div>
    ...
    return (
        <div className={`dashboard-layout lang-${appLang}`}>
            ...
            <main className="main-content">
                ...
                <header className="header-top">
                    <div className="dashboard-controls" style={{ display: 'flex', alignItems: 'center', gap: '15px' }}>
                        ...
                        <div className="in-app-lang" style={{ display: 'flex', gap: '8px', background: 'rgba(255,255,255,0.7)', padding: '5px 10px', borderRadius: '30px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)' }}>
                            <button className={`lang-mini-btn ${appLang === 'vi' ? 'active' : ''}`} onClick={() => setAppLang('vi')} style={{ border: 'none', background: 'none', cursor: 'pointer', outline: 'none', transition: '0.3s' }}>
                                <img src="https://flagcdn.com/w20/vn.png" alt="VI" />
                            </button>
                            <button className={`lang-mini-btn ${appLang === 'en' ? 'active' : ''}`} onClick={() => setAppLang('en')} style={{ border: 'none', background: 'none', cursor: 'pointer', outline: 'none', transition: '0.3s' }}>
                ...
                {view === 'home' && <HomeView onNavigate={setView} appLang={appLang} />}
                ...
                {view === 'farms' && <div className="view-container"><h2>{t.farms}</h2><button onClick={() => setView('home')} className="btn-back"><i className="fas fa-arrow-left"></i> {t.back}</button></div>}
